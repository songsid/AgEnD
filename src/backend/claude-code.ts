...
export class ClaudeCodeBackend implements CliBackend {
  ...
  getErrorPatterns(): ErrorPattern[] {
    return [
      ...
      // Claude Code pauses the session (rather than merely warning) when the
      // current usage window is exhausted.  Keep this as a complete line
      // match: the warning is also visible in the pane scrollback, and prose
      // discussing only part of it must not turn into a pause notification.
      // The time is deliberately captured so the channel notice tells the
      // operator when Claude is expected to continue.
      {
        pattern: /^[ \t]*⚠(?:\uFE0F)?[ \t]+Usage limit reached[ \t]*·[ \t]*continuing automatically at[ \t]+((?:(?:[01]?\d|2[0-3]):[0-5]\d(?:[ \t]*[ap]m)?|(?:0?[1-9]|1[0-2])(?::[0-5]\d)?[ \t]*[ap]m))[ \t]*·[ \t]*esc to cancel[ \t]*$/im,
        type: "quota",
        action: "pause",
        message: "Claude Code usage limit reached — paused until the next usage window",
        formatMessage: match => `Claude Code usage limit reached — paused; continuing automatically at ${match[1]}`,
