...
const enMessages = {
    ...
    "classic.approve_task": "Approved: add `{0}` to classicBot.yaml `defaults.allowed_guilds`. Keep it a QUOTED STRING — a Discord id is too large for a YAML integer. Change nothing else, then confirm here.",
    "classic.approve_task_admin": "Approved: add `{0}` to classicBot.yaml `defaults.allowed_guilds` AND `{1}` to `defaults.admin_users`. Keep both as QUOTED STRINGS — a Discord id is too large for a YAML integer. Change nothing else, then confirm here.",
...
const zhTWMessages: { [K in keyof typeof enMessages]: string } = {
    ...
    "classic.approve_task": "已核准：把 `{0}` 加進 classicBot.yaml 的 `defaults.allowed_guilds`。**必須加引號存成字串** —— Discord ID 太大，存成 YAML 整數會失去精度。其他欄位一律不要動，完成後回報。",
    "classic.approve_task_admin": "已核准：把 `{0}` 加進 classicBot.yaml 的 `defaults.allowed_guilds`，並把 `{1}` 加進 `defaults.admin_users`。**兩者都必須加引號存成字串** —— Discord ID 太大，存成 YAML 整數會失去精度。其他欄位一律不要動，完成後回報。",
