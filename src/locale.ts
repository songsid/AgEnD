...
const enMessages = {
    ...
    "ctx.model": "Model: {0}",
    "ctx.effort": "Effort: {0}",
    "ctx.effort_configured": "Effort: {0} (configured — the CLI may have been changed since)",
    ...
    "restart.full_preparing": "🔄 Full process reload requested — preparing a tracked restart...",
    "restart.full_launch_failed": "❌ Full process reload could not start; the current fleet is still running. Check the fleet log and retry.",
    "restart.full_busy": "⚠️ An update, full reload, or fleet shutdown is already in progress. This reload was not started.",
    "restart.progress.stopping": "🔄 Full reload — waiting for instances to idle and stopping fleet... ({0}s)",
    "restart.progress.starting": "🚀 Full reload — starting the new fleet process... ({0}s)",
    "restart.progress.instances": "⏳ Full reload — {0}/{1} instances started ({2}s)",
    "restart.progress.complete": "✅ Full process reload complete — v{0} ({1}s)",
    "restart.progress.complete_summary": "✅ Full process reload complete — v{0}, {1}/{2} instances running ({3}s)",
    "restart.progress.failed": "❌ Full process reload failed: {0} ({1}s). The current fleet is still running.",
    ...
    "inst.codex_legacy_session": "⚠️ `{0}` started a new Codex session after upgrade. Its older conversation is preserved on disk but was not automatically resumed: no instance-owned session ID could be verified. To recover it, stop this instance, verify the old UUID, run `agend fleet codex-resume {0} <session-id>`, then start this instance. AgEnD will never guess with `--last`.",
    "inst.codex_session_unconfirmed": "⚠️ `{0}` has multiple open Codex conversations, but the current chat could not be verified. The live pane is left running; automatic resume is held so it cannot reopen the wrong chat. Check the pane and explicitly attach the correct session ID before restarting.",
    ...
    "inst.muse_relay_exhausted": "⚠️ {0}: Muse usage relay failed and did not recover — usage is unavailable, and the CLI is being moved back to a direct connection (session preserved).",
    ...
    "inst.reply_drop_retrying": "⚠️ The agent ended this turn without sending a reply. AgEnD will ask it once to send a concise conclusion.",
    "inst.reply_drop_unknown": "⚠️ The agent's reply could not be confirmed as delivered. To avoid a duplicate, AgEnD will not resend it automatically; ask again if needed.",
    "inst.reply_drop_unrecovered": "⚠️ The agent still did not send a reply after one recovery attempt. A long conversation may have degraded its reply tool. Save important context to soul.md, then run /clear.",
    ...
    "restart.usage": "Usage: /restart or /restart full",
    ...
    "login.status_installed": "Installed",
    "login.status_configured": "Configured",
    "login.status_auth": "Auth",
    "login.status_unsupported": "Unsupported",
    "login.none_available": "⚠️ No installed or configured backend with a remote login flow. Install one with `/install-cli <backend>`.",
    "login.no_remote_flow": "⚠️ Backend \"{0}\" has no remote login flow. Run its CLI login command on the host.",
    ...
    "login.completed": "✅ {0} login completed. Bringing the affected instances back so they pick up the new credentials…",
    "login.recovered": "✅ {0} is ready. Woke: {1}. Restarted (to reload credentials): {2}. You can talk to them again now.",
    "login.recover_pending": "⏳ {0} login is done, but these are still restarting after {1}s: {2}. They will come back on their own — no need to restart anything by hand.",
    "login.recover_failed": "✅ {0} login completed, but bringing the instances back failed: {1}. Use /restart if they do not return.",
    "login.offer": "Re-login to {0} from here:",
    "login.offer_action": "🔑 Log in to {0}",
    "login.none": "none",
    ...
    "login.still_valid_precommand": "⚠️ {0} authentication currently works. Re-login runs `{1}` first, so every running {0} instance loses its sign-in until the new login completes (they restart automatically). Continue?",
    "login.web_started": "🔐 {0} login started in a browser terminal. The link is posted here; the access token is sent to you privately — you need both.",
    "login.web_link": "🔐 {0} login — open this link in any browser (it runs only `{2}`, no shell; closes in {1} min or when the command exits). The link alone opens nothing: paste the access token I sent you privately.\n{3}\nPlain HTTP: reach it over SSH forwarding, tailscale, or a TLS proxy — not an untrusted network.",
    "restart.settings_triggered": "🔄🔄 AgEnD is restarting — requested from the Settings page.",
    "login.web_token": "🔑 {0} login — your one-time access token (paste it into the terminal page; do not forward):",
    "login.web_token_dm_failed": "⚠️ I could not send you the access token privately (DMs disabled, or you have never messaged this bot). The token is never posted in a channel. Enable DMs / message the bot once, then press Resend.",
    "login.web_resend": "Resend token",
    "login.web_token_resent": "🔑 Access token re-sent privately.",
    "login.web_token_already_used": "The access token was already redeemed — the terminal is open in a browser.",
    "login.web_disabled": "⚠️ The web terminal is disabled (web_terminal.enabled: false). Enable it, or set login.mode: relay.",
    "login.web_cleanup_failed": "⚠️ {0}: the dedicated tmux server could not be confirmed dead — check `tmux -L agend-term-* ls` on the host.",
    "login.web_suggest_relogin": "→ Run /login again and choose Re-login: it signs the CLI out first.",
    "login.web_suggest_check_args": "→ Check the Start URL / Region you entered and run /login again.",
    "login.web_code_not_needed": "This login runs in a browser terminal — type directly in the page; /login code is not needed.",
    "login.web_confirm": "🔐 About to open a browser terminal for {0} login: a temporary link (a few minutes, closes when the command exits) that runs ONLY the login command — no shell. The link alone opens nothing; the access token is sent to you privately. Do not forward either message. Continue?",
    "login.web_confirm_go": "I understand, open",
    "login.web_confirm_failed": "could not post the confirmation buttons",
    "login.web_rate_limited": "⚠️ Too many login sessions: at most {0} per {1} minutes per admin. Try again later.",
    "login.web_flow_not_allowed": "⚠️ {0} is not cleared for the browser terminal (its login screen has not been reviewed for shell escapes). Set login.mode: relay, or ask a maintainer to review the flow.",
    "login.web_link_failed": "the terminal link could not be delivered to this chat — nothing was opened",
    "login.web_token_failed": "the access token could not be delivered privately and no resend button could be posted — nothing was opened",
    "login.web_token_resend_failed": "❌ The access token still could not be delivered privately, so the {0} login session was closed. Enable DMs (or message the bot once) and run /login again.",
    "login.web_shutting_down": "⚠️ The fleet is shutting down — no login window can be opened right now.",
    "login.remote_unsupported_agent_cli": "⛔ Remote login for {0} is not supported in this version. `{1}` is the full agent CLI with no isolated login command — signing in means running the whole agent, which breaks the terminal's \"one login command, no shell\" boundary. Run `{1}` directly on the host to sign in; remote login will follow once upstream ships a dedicated login command.",
    ...
    "install.success": "✅ {0} installed and verified (`{1}`). Next: `/login {0}`.",
    ...
    "slash.restart": "Restart instances; use full for a process reload",
    ...
    "slash.option.restart_mode": "Optional full process reload",
    ...
    "usage.value.unlimited": "♾️ Unlimited",
    ...
    "model.refreshing": "⏳ Fetching the latest model list…",
    "model.refresh": "🔄 Refresh models",
    "model.refreshed": "🔄 Model list refreshed.",
    "model.refresh_failed": "⚠️ Could not refresh the model list — showing the last known one.",
    ...
    "fleet.topic_probe_unknown": "⚠️ Could not verify {0}'s topic through adapter {1} for {2} consecutive checks. The instance, configuration, and working directory were retained.",
    "fleet.topic_probe_bulk": "⚠️ {0} topics appeared missing in one check. The result was treated as an outage; no instance or working directory was changed.",
    "fleet.topic_quarantined": "⚠️ {0}'s topic ({1}) was confirmed missing. Its route was quarantined, but its configuration and working directory were retained. Rebind it or delete it explicitly.",
...
const zhTWMessages: { [K in keyof typeof enMessages]: string } = {
    ...
    "ctx.model": "Model：{0}",
    "ctx.effort": "Effort：{0}",
    "ctx.effort_configured": "Effort：{0}（設定值，之後可能在 CLI 內被改過）",
    ...
    "restart.full_preparing": "🔄 已要求完整重載程序 — 正在建立可追蹤的重啟交接...",
    "restart.full_launch_failed": "❌ 無法啟動完整重載；目前 Fleet 仍在運行。請檢查 fleet log 後重試。",
    "restart.full_busy": "⚠️ 更新、完整重載或 Fleet 關閉程序已在進行中。本次重載未啟動。",
    "restart.progress.stopping": "🔄 完整重載中 — 等待 Agent 閒置並停止 Fleet...（{0}s）",
    "restart.progress.starting": "🚀 完整重載中 — 正在啟動新的 Fleet 程序...（{0}s）",
    "restart.progress.instances": "⏳ 完整重載中 — {0}/{1} 個 Agent 已啟動（{2}s）",
    "restart.progress.complete": "✅ 完整重載程序完成 — v{0}（{1}s）",
    "restart.progress.complete_summary": "✅ 完整重載程序完成 — v{0}，{1}/{2} 個 Agent 運行中（{3}s）",
    "restart.progress.failed": "❌ 完整重載程序失敗：{0}（{1}s）。目前 Fleet 仍在運行。",
    ...
    "inst.codex_legacy_session": "⚠️ `{0}` 升級後以新的 Codex session 啟動。舊對話仍保留在磁碟，但因無法驗證其 instance 專屬 session ID，未自動續接。若要恢復，請先停止此 instance、核對舊 UUID，再執行 `agend fleet codex-resume {0} <session-id>`，最後重新啟動。AgEnD 絕不以 `--last` 猜測。",
    "inst.codex_session_unconfirmed": "⚠️ `{0}` 同時開著多個 Codex 對話，但目前對話無法驗證。現有 pane 會繼續運作；自動 resume 已暫停，避免續到錯誤對話。請先核對 pane，人工指定正確 session ID 後再重啟。",
    ...
    "inst.muse_relay_exhausted": "⚠️ {0}：Muse 用量 relay 失效且無法恢復 —— 用量暫不可用，CLI 正移回直接連線（session 保留）。",
    ...
    "inst.reply_drop_retrying": "⚠️ 代理已結束這一輪，但沒有送出回覆。AgEnD 會要求它補送一次精簡結論。",
    "inst.reply_drop_unknown": "⚠️ 代理這一輪的回覆無法確認是否送達。為避免重複訊息，AgEnD 不會自動重送；如有需要，請再詢問一次。",
    "inst.reply_drop_unrecovered": "⚠️ 代理在一次補送後仍未送出回覆。長對話可能已使回覆工具退化；請先把重要內容保存到 soul.md，再執行 /clear。",
    ...
    "restart.usage": "用法：/restart 或 /restart full",
    ...
    "login.status_installed": "已安裝",
    "login.status_configured": "已設定",
    "login.status_auth": "可登入",
    "login.status_unsupported": "不支援",
    "login.none_available": "⚠️ 沒有已安裝或已設定、且支援遠端登入的 Backend。請用 `/install-cli <backend>` 安裝。",
    "login.no_remote_flow": "⚠️ Backend「{0}」沒有遠端登入流程。請在主機上執行它自己的 CLI 登入指令。",
    ...
    "login.completed": "✅ {0} 登入完成。正在把受影響的 instance 帶回來，讓它們載入新憑證…",
    "login.recovered": "✅ {0} 已就緒。已喚醒：{1}。已重啟（重新載入憑證）：{2}。現在可以正常對話了。",
    "login.recover_pending": "⏳ {0} 登入已完成，但這些還在重啟中（已等 {1} 秒）：{2}。它們會自己回來，不需要手動重啟。",
    "login.recover_failed": "✅ {0} 登入完成，但把 instance 帶回來時失敗了：{1}。若它們沒有回來，請用 /restart。",
    "login.offer": "直接從這裡重新登入 {0}：",
    "login.offer_action": "🔑 登入 {0}",
    "login.none": "無",
    ...
    "login.still_valid_precommand": "⚠️ {0} 目前的登入仍有效。重新登入會先執行 `{1}`，所有執行中的 {0} instance 會暫時失去登入，直到新登入完成（會自動重啟）。繼續？",
    "login.web_started": "🔐 已在瀏覽器終端啟動 {0} 登入。連結發在這裡；存取 token 會私訊給你 —— 兩者都需要。",
    "login.web_link": "🔐 {0} 登入 —— 用任何瀏覽器打開此連結（裡面只跑 `{2}`，沒有 shell；{1} 分鐘後或指令結束即關閉）。光有連結打不開任何東西：請貼上我私訊給你的存取 token。\n{3}\n純 HTTP：請透過 SSH 轉發、tailscale 或 TLS 反向代理到達，勿在不可信網路使用。",
    "restart.settings_triggered": "🔄🔄 AgEnD 正在重新啟動 —— 由 Settings 頁面觸發。",
    "login.web_token": "🔑 {0} 登入 —— 你的一次性存取 token（貼到終端頁面；請勿轉傳）：",
    "login.web_token_dm_failed": "⚠️ 無法私訊你存取 token（DM 關閉，或你從未私訊過這個 bot）。token 絕不會發在頻道裡。請開啟 DM／先私訊 bot 一次，再按「重送 token」。",
    "login.web_resend": "重送 token",
    "login.web_token_resent": "🔑 存取 token 已重新私訊。",
    "login.web_token_already_used": "存取 token 已兌換 —— 終端已在瀏覽器開啟。",
    "login.web_disabled": "⚠️ 瀏覽器終端已停用（web_terminal.enabled: false）。請啟用，或設 login.mode: relay。",
    "login.web_cleanup_failed": "⚠️ {0}：專屬 tmux server 無法確認已關閉 —— 請到主機執行 `tmux -L agend-term-* ls` 檢查。",
    "login.web_suggest_relogin": "→ 再執行一次 /login 並選「重新登入」：會先把 CLI 登出。",
    "login.web_suggest_check_args": "→ 檢查你輸入的 Start URL／Region，再執行一次 /login。",
    "login.web_code_not_needed": "這次登入在瀏覽器終端進行 —— 直接在頁面裡輸入；不需要 /login code。",
    "login.web_confirm": "🔐 即將為 {0} 登入開一條瀏覽器終端：臨時連結（幾分鐘、指令結束即關），裡面只跑登入指令、沒有 shell。光有連結打不開任何東西；存取 token 會私訊給你。兩則訊息都請勿轉傳。繼續？",
    "login.web_confirm_go": "我了解，開啟",
    "login.web_confirm_failed": "無法貼出確認按鈕",
    "login.web_rate_limited": "⚠️ 登入 session 太頻繁：每位 admin 每 {1} 分鐘最多 {0} 次。請稍後再試。",
    "login.web_flow_not_allowed": "⚠️ {0} 尚未核可使用瀏覽器終端（其登入畫面未經 shell 逃逸審查）。請設 login.mode: relay，或請維護者審查該 flow。",
    "login.web_link_failed": "終端連結無法送達此聊天 —— 未開啟任何東西",
    "login.web_token_failed": "存取 token 無法私訊送達、也無法貼出重送按鈕 —— 未開啟任何東西",
    "login.web_token_resend_failed": "❌ 存取 token 仍無法私訊送達，{0} 登入 session 已關閉。請開啟 DM（或先私訊 bot 一次）後再執行 /login。",
    "login.web_shutting_down": "⚠️ fleet 正在關閉 —— 目前無法開啟登入視窗。",
    "login.remote_unsupported_agent_cli": "⛔ {0} 的遠端登入在這個版本不支援。`{1}` 是完整的 agent CLI、沒有隔離的登入子指令 —— 登入等於跑整個 agent，違反瀏覧器終端「只跑登入指令、沒有 shell」的邊界。請暫時在主機上直接執行 `{1}` 登入；等上游提供專用 login 指令後再支援。",
    ...
    "install.success": "✅ {0} 已安裝並驗證（`{1}`）。下一步：`/login {0}`。",
    ...
    "slash.sysinfo": "系統診斷",
    "slash.restart": "重啟 Agent；full 可完整重載程序",
    ...
    "slash.option.restart_mode": "選填：完整重載程序",
    "slash.usage": "AI 訂閱用量",
    ...
    "model.refreshing": "⏳ 正在抓取最新模型清單…",
    "model.refresh": "🔄 重新整理模型",
    "model.refreshed": "🔄 已更新模型清單。",
    "model.refresh_failed": "⚠️ 無法更新模型清單，以下是上次的清單。",
    ...
    "fleet.topic_probe_unknown": "⚠️ 已連續 {2} 次無法透過 adapter {1} 確認 {0} 的 topic 狀態。instance、設定與工作目錄均已保留。",
    "fleet.topic_probe_bulk": "⚠️ 單次檢查中有 {0} 個 topic 看似消失，已視為服務異常；沒有修改任何 instance 或工作目錄。",
    "fleet.topic_quarantined": "⚠️ 已確認 {0} 的 topic（{1}）不存在。路由已隔離，但設定與工作目錄均已保留；請重新綁定或明確刪除。",
