...
**Topic mode（推薦）：** Fleet manager 擁有共用 channel adapter（單一 bot poller），按 threadId 路由到各 daemon instance。

```
┌──────────────────────────────────────────────────────┐
│  Fleet Manager                                        │
│  ├── Shared Channel Adapter (1 bot, 1 poller)        │
│  │     └── Telegram Forum Group                       │
│  │          ├─ Topic #42 → route to daemon-A          │
│  │          ├─ Topic #87 → route to daemon-B          │
│  │          └─ unbound topic → reply "not bound"      │
│  │                                                    │
│  ├── Topic Router (threadId → instance IPC)           │
│  │     ├─ channel-a.sock ──→ daemon-A                │
│  │     ├─ channel-b.sock ──→ daemon-B                │
│  │     └─ channel-c.sock ──→ daemon-C                │
│  │                                                    │
│  ├── Tmux Session "ccd"                               │
│  │     ├─ window @0: claude (project-a)               │
│  │     ├─ window @1: claude (project-b)               │
│  │     └─ window @2: claude (project-c)               │
│  │                                                    │
│  └── [daemon-A] [daemon-B] [daemon-C]  ← child procs │
└──────────────────────────────────────────────────────┘
```

每個 daemon instance 內部（不持有 adapter）：

```
┌──────────────────────────────────────────┐
│  Daemon Instance                          │
│  ├── IPC client (channel.sock)           │
│  │     ↕ fleet manager 的 adapter        │
...
│  ├── Tool Tracker + Message Queue proxy  │
│  ├── Context Guardian                    │
│  └── Memory Layer → SQLite               │
└──────────────────────────────────────────┘
```

**DM mode（向下相容）：** 每個 instance 擁有自己的 adapter + bot，無需 fleet manager 路由。

## Decision: Multi-instance Model

每個 daemon instance = 1 個專案 + 1 個 Claude session。

**Topic mode：** Fleet manager 持有共用 channel adapter（**一個 bot token 只能有一個 polling connection**），按 threadId 分發 inbound 訊息到對應的 daemon IPC socket。Daemon instance 不直接接觸 adapter。

**DM mode：** 每個 daemon instance 擁有自己獨立的 adapter + bot token。

採用 **多 process** 而非單 process 管多專案：
- Claude Code 一次只能跑一個 session，多專案必然是多個 `claude` process
- 故障隔離 — 一個專案掛了不影響其他
- Fleet manager 管理共用 adapter + tmux session + daemon child processes
...
Topic binding 唯一定義在 per-instance 的 `topic_id` 欄位（single source of truth）。Fleet manager 啟動時從所有 instance configs 建立 routing table。
...
instances:
  project-a:
    ...
    topic_id: 42                       # ← binding 定義在這裡
  project-b:
    working_directory: /path/to/project-b
    topic_id: 87
```

Fleet manager 啟動時建立 routing table：`{ 42: "project-a", 87: "project-b" }`。
...
interface ChannelAdapter {
  ...
  // Promise rejects if the approval message could not be sent
  // callback fires when user responds
  sendApproval(prompt: string,
    callback: (decision: 'approve' | 'deny') => void,
    signal?: AbortSignal): Promise<ApprovalHandle>;
...
`MessageQueue` 是 `ChannelAdapter` 的**內部實作細節**。Adapter 的 `sendText()`/`editMessage()` 等公開方法內部透過 queue 發送，呼叫者不感知 queue 的存在。`Promise<SentMessage>` 在訊息**實際送出**後 resolve（不是入隊時）。

`ToolTracker` 是唯一直接使用 `QueuedMessage` 低階介面的元件（需要 `status_update`/`status_clear` 語意）。
...
`ToolTracker` 接收 `TranscriptMonitor` 的事件，透過 adapter 的 `MessageQueue` 低階介面發送/更新 status 訊息：
...
class ToolTracker {
  ...
  // TranscriptMonitor → ToolTracker
  onToolUse(toolName: string, input: unknown): void;   // → enqueue status_update
  onToolResult(toolName: string, output: unknown): void; // → enqueue status_update (edit)

  // 內部追蹤 current status message ID for in-place editing
...
class TmuxManager {
  ...
  // Session-level lifecycle (static — called by fleet manager)
  static async ensureSession(name: string): Promise<void>;    // create if not exists
  ...
  // Window-level lifecycle (per-instance)
  ...
  async pipeOutput(logPath: string): Promise<void>;   // tmux pipe-pane → file (raw terminal)
...
替代現有 `cli.ts` 的 `readFileSync` 整檔讀取，改為只讀增量。

**讀取來源：** Claude Code 的結構化 JSONL transcript 檔案（`~/.claude/projects/.../<session>.jsonl`），路徑從 `statusline.json` 取得。**不是** tmux `pipe-pane` 的 raw terminal output。

`pipe-pane` output (`output.log`) 僅供 `TmuxPromptDetector` 使用（偵測權限 prompt 的 raw terminal 文字）。
...
class TranscriptMonitor extends EventEmitter {
  ...
  // 找到 transcript JSONL 路徑（從 statusline.json 的 transcript_path 或 project dir scan）
  async resolveTranscriptPath(): Promise<string | null>;

  // 讀取新增內容（從 byteOffset 開始）
  async pollIncrement(): Promise<string>;

  // 解析 JSONL 行，emit typed events
  // 'tool_use'        → ToolTracker.onToolUse()
  // 'tool_result'     → ToolTracker.onToolResult()
  // 'assistant_text'  → MessageQueue (content)
  // 'channel_message' → logging
}
```
...
偵測 tmux `pipe-pane` raw output 中的權限 prompt（`"1.Yes"` + `"3.No"` pattern），呼叫 `messageBus.requestApproval()`。

```typescript
class TmuxPromptDetector {
  constructor(
    private outputLogPath: string,     // pipe-pane output file
    private tmux: TmuxManager,
    private approvalFn: (prompt: string) => Promise<ApprovalResponse>,
    private logger: Logger,
  ) {}

  startPolling(intervalMs?: number): void;  // default 2000ms, tail output.log
  stop(): void;

  // 偵測到 prompt → call approvalFn → send "1" or "3" via tmux.sendKeys()
}
```

## Data Flow — Wiring

`daemon.ts` 負責接線，完整資料流：

```
[Inbound — Topic mode]
  Telegram API → Fleet Manager's Adapter
    → route by threadId → daemon IPC (channel.sock)
      → MCP Server → channel protocol push → Claude

[Inbound — DM mode]
  Telegram API → Daemon's own Adapter
    → MCP Server → channel protocol push → Claude

[Outbound — Claude tool calls]
  Claude → MCP Server (reply/react/edit)
    → IPC → daemon → adapter.sendText() (internally queued)

[Transcript Monitoring]
  Claude writes JSONL → TranscriptMonitor (byte-offset poll)
    → emit 'tool_use'      → ToolTracker.onToolUse()  → adapter queue (status_update)
    → emit 'tool_result'   → ToolTracker.onToolResult() → adapter queue (status_update edit)
    → emit 'assistant_text' → adapter queue (content, triggers push notification)

[Approval]
  PreToolUse hook → HTTP POST → ApprovalServer
    → messageBus.requestApproval() → adapter.sendApproval() → user clicks → response

  Tmux raw output → TmuxPromptDetector
    → approvalFn() → messageBus.requestApproval() → ... → tmux.sendKeys("1"/"3")

[Context Rotation]
  statusline.json → ContextGuardian → emit 'rotate'
    → daemon clears session, restarts tmux window
```
...
**IPC 重連機制：** 因為 tmux 讓 Claude（和 MCP server）在 daemon crash 後繼續運行，MCP server.js 必須實作重連邏輯：斷線時指數退避重連（1s → 2s → 4s，max 30s）。斷線期間 MCP tool calls 返回 error response（不 hang）。IPC protocol 包含 heartbeat ping/pong（每 10 秒）。

**`${CLAUDE_PLUGIN_ROOT}` 展開：** 需驗證 Claude Code 的 plugin loader 是否支援此變數展開。Fallback：`server.js` 使用 `__dirname` 定位自身，socket 路徑使用 well-known convention（`~/.claude-channel-daemon/instances/<name>/channel.sock`）而非環境變數。
...
instances:
  project-a:
    working_directory: /path/to/project-a
    topic_id: 42                         # ← topic binding (single source of truth)
    context_guardian:
  ...
  project-c:
    ...
    channel:                             # per-instance override → DM mode
      type: telegram
      mode: dm
...
- 頂層 `channel` 是共用的（topic mode），instance 可 override 自己的 `channel`（DM mode）
- `topic_id` 定義在 instance 層級（single source of truth）— fleet manager 啟動時掃描所有 instance 建立 routing table

**Backward compatibility：** 單機模式 `ccd start` 仍讀取 `config.yaml`（舊格式）。內部將 `DaemonConfig` 映射為 `InstanceConfig`。`ccd start` 也使用 tmux（node-pty 完全移除）。舊的 `channel_plugin` 欄位觸發 legacy 模式：使用外部 plugin 而非內建 MCP server。
...
- **Instance-scoped data directory**: `~/.claude-channel-daemon/instances/<name>/`
  ...
  - `output.log` — tmux pipe-pane raw terminal output（僅供 TmuxPromptDetector 使用）
