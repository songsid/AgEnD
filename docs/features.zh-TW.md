...
Agy 拒絕在隱藏路徑（如 `~/.agend/`）下運作。當 workspace 位於隱藏路徑時，AgEnD 會在 `~/agend-workspaces/<instanceName>/` 建立真實目錄作為 CWD。Instructions 寫入該目錄下的 `.agents/agents.md`。
...
Grok 使用 x.ai OAuth device flow。首次啟動時：
1. TUI 顯示裝置碼（例如 `5M6B-584D`）
2. 使用者在瀏覽器開啟網址並輸入裝置碼
3. 認證資訊保存在 `~/.grok/`，跨重啟持續有效

Headless 環境（SSH、Docker）需在另一台機器手動開啟網址完成授權。

### 已知行為

- **Context 顯示** — 以 `已用 / 總量` token 呈現（例如 `12K / 500K`）；AgEnD 的 `/ctx` 與 `agend ls` 會解析成百分比
- **Cancel 鍵** — Ctrl+C（中斷生成；已對實機驗證）
- **Session 恢復** — `--resume <session-id>`（AgEnD 依 working directory 自動保存 session id）。**不使用** `--continue` — 沒有前一個 session 時它會直接退出
- **Session 隔離** — session 依 working directory 分開存放（`~/.grok/sessions/<encoded-cwd>/`），多個 grok instance 不會互相干擾
- **Tool approval** — 以 `--always-approve` 自動核准工具執行

## 自動暫停與喚醒（Auto-Pause & Wake）

閒置的 instance 可自動暫停以節省資源，收到訊息時自動喚醒。

### 運作方式

- **自動暫停**：instance 閒置超過 `auto_pause_after` 分鐘後自動暫停（tmux window 保留，CLI 掛起）
- **自動喚醒**：用戶發訊息給暫停的 instance 時，自動喚醒（約 30 秒恢復）
- **工作保護**：正在回覆的 instance 不會被暫停

### 設定

```yaml
defaults:
  auto_pause_after: 0    # 0 = 停用（預設）。設定分鐘數 > 0 啟用。

instances:
  my-agent:
    auto_pause_after: 30  # 每個 instance 可覆寫：30 分鐘閒置後暫停
```

也可在 Settings 網頁設定：**Runtime & Resources → auto_pause_after**。

### 手動控制

| 指令 | 平台 | 說明 |
|------|------|------|
| `/pause` | DC slash / TG 指令 | 手動暫停 instance（僅 admin） |
| `/wake` | DC slash / TG 指令 | 手動喚醒暫停的 instance（僅 admin） |

### 狀態可見性

- `agend ls` → Status 欄顯示 "Paused" + ⏸ 圖示
- `/status` → ⏸ 圖示
- Settings 頁面 → 暫停的 instance 顯示 "Wake" 按鈕
- MCP `list_instances` → `status: "paused"`

### 行為說明

- 暫停的 instance 不佔 CPU/RAM
- 訊息到達自動喚醒 — 用戶不需手動 wake
- 跨 instance 訊息（`send_to_instance`）也會觸發喚醒
- Schedule 觸發前會先喚醒
- 喚醒約 30 秒（恢復 session context）

### 建議值

| 場景 | 建議值 |
|------|--------|
| 個人開發（少量 instance） | `0`（停用）— 保持常駐 |
| 團隊 fleet（10+ instances） | `30`（30 分鐘）— 節省資源 |
| 大型 fleet（控制成本） | `10` — 積極暫停 |
| 必須常駐 | `0` 或不設 |
...
`agend ls` 使用 PSS（Proportional Set Size）從 `/proc/<pid>/smaps_rollup` 取值，而非 RSS。避免跨 process tree 重複計算共用函式庫頁面，更準確反映實際記憶體消耗。非 Linux 系統時 fallback 回 RSS。
